...
### Roles
Three tiers, stored as plain text in `users.role`: `user` (default), `admin`, `owner`. There is exactly one `owner` — `hanzlailyas345@gmail.com`, set by a one-time `UPDATE` (see the migration block below), never assignable through the app. `src/lib/roles.ts` is the single source of truth for who can act on whom (`canManageRole`, `canSetRole`, `canDeleteUser`, with a self-check at `src/lib/roles.test.ts` — run via `node --test src/lib/roles.test.ts`): owner can change or delete anyone but the owner and themself; admin can only touch plain `user` accounts and can only ever promote them to `admin`, never demote — so an account that reaches `admin` can only be walked back by the owner. Both `middleware.ts` (gates `/api/admin/*`) and `dashboard/page.tsx` (gates the `AdminPanel`/`AuditLogPanel` UI) treat `admin` and `owner` as equally allowed to *enter* the admin area; the fine-grained per-target rules live in `roles.ts` and are enforced server-side in the route handlers, not just hidden in the UI.
...
UPDATE users SET role = 'owner' WHERE email = 'hanzlailyas345@gmail.com';
